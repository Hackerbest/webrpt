...
const playerOriginalParent = player ? player.parentElement : null;
const playerOriginalNextSibling = player ? player.nextElementSibling : null;
...
function applyVolume(value) {
  ...
  if (isIOSDevice() && !warnedMobileVolume) {
    warnedMobileVolume = true;
    console.info("iOS browsers control audio volume with the device buttons, not audio.volume.");
  }
}

function updateProductPlayButtons() {
  document.querySelectorAll("[data-product-play-id]").forEach(btn => {
    const buttonId = `mock-${btn.dataset.productPlayId}`;
    const active = playingId === buttonId;
...
function renderPlaylistTabs() {
  ...
  const categories = [...new Set(
    mockSongs
      .map(song => String(song.category || "").trim())
      .filter(Boolean)
      .map(value => {
...
function detailPreview(text, maxLength = 90) {
...
function renderProductList(listy = "all") {
  ...
  const matchesCategory = (songCategory, selected) => {
    const categoryNum = Number(songCategory);
    const selectedNum = Number(selected);

    // e.g. selected 180 matches 180-189
    if (Number.isFinite(categoryNum) && Number.isFinite(selectedNum)) {
      return categoryNum >= selectedNum && categoryNum <= selectedNum + 9;
    }

    return String(songCategory) === String(selected);
  };

  const filteredSongs = listy === "all"
    ? mockSongs
    : mockSongs.filter(song => matchesCategory(song.category, listy));
  ...
  productList.innerHTML = filteredSongs.map(song => {
    ...
    const preview = detailPreview(song.details);
    const coverNode = song.coverUrl
      ? `<img src="${song.coverUrl}" alt="${song.title}" loading="lazy" onerror="this.style.display='none';this.parentElement.classList.add('cover-fallback')">`
      : "&#9835;";
    return ` 
      <article class="product-card clickable-product" onclick="showProductDetail(${song.id})">
        <div class="product-row">
          ...
          <div class="product-info">
            <h3>${song.title}</h3>
            <p>${song.artist}</p>
            ${preview ? `<p class="product-preview">${escapeHtml(preview)}</p>` : ""}
          ...
          <div class="product-actions">
            ...
            <button type="button" class="play-btn ${active ? "active" : ""}" data-product-play-id="${song.id}" onclick="return playProductFromButton(event, ${song.id})">${active ? "❚❚" : "▶"}</button>
            <button class="buy-btn" onclick="event.stopPropagation();showProductDetail(${song.id})">ดูรายละเอียด</button>
          </div>
        </div>
      </article>`;
  }).join("");
...
function showProductDetail(id) {
  const song = mockSongs.find(item => Number(item.id) === Number(id));
  const area = document.getElementById("productDetailArea");
  if (!song || !area) return;

  const details = escapeHtml(song.details || "ยังไม่มีรายละเอียดเพิ่มเติม").replace(/\n/g, "<br>");
  const coverNode = song.coverUrl
    ? `<img src="${song.coverUrl}" alt="${escapeHtml(song.title)}">`
    : `<div class="product-detail-placeholder">&#9835;</div>`;

  area.innerHTML = `
    <article class="product-detail-card">
      <div class="product-detail-cover">${coverNode}</div>
      <div class="product-detail-content">
        <span class="category">${escapeHtml(song.category || "-")}</span>
        <h2>${escapeHtml(song.title)}</h2>
        <p class="product-detail-artist">${escapeHtml(song.artist)}</p>
        <div class="product-detail-meta">
          <span>${escapeHtml(song.duration || "0:00")}</span>
          <strong>${escapeHtml(song.price)} บาท</strong>
        </div>
        <h3>รายละเอียดสินค้า</h3>
        <div class="product-detail-text">${details}</div>
        <div class="product-detail-actions">
          <button class="play-btn product-detail-play ${playingId === `mock-${song.id}` ? "active" : ""}" data-product-play-id="${song.id}" type="button" onclick="return playProductFromButton(event, ${song.id})">
            ${playingId === `mock-${song.id}` ? "❚❚" : "▶"}
          </button>
          <button class="buy-btn" type="button">ซื้อเลย</button>
        </div>
        <div class="product-detail-player-slot"></div>
      </div>
    </article>
  `;
...
function playProductFromButton(event, id) {
  if (event) {
    ...
    if (mockSongs.length === 0) {
      songGrid.innerHTML = `
        <div class="card empty-box">
          ...
          <p>ยังไม่มีเพลงในระบบ หรือกำลังโหลดข้อมูล</p>
        </div>
      `;
    } else {
      songGrid.innerHTML = mockSongs.map(song => {
        ...
        return ` 
          <article class="song-card clickable-product" style="--i:${song.id}" onclick="showProductDetail(${song.id})">
            <div class="cover">
              ${coverNode}
              ${active ? `<div class="bouncing"><span></span><span></span><span></span><span></span><span></span></div>` : ""}
            </div>
...
async function loadProductsFromSupabase() {
  try {
    ...
    mockSongs = rows.map(item => ({
      ...
      details: item.product_details || item.details || item.description || ""
    }));
...
function updatePlayer() {
  ...
  updateProductPlayButtons();
  syncPlayerPlacement();
}

function syncPlayerPlacement() {
  if (!player || !playerOriginalParent) return;

  const mobile = window.matchMedia("(max-width: 640px)").matches;
  const detailActive = document.getElementById("productDetail")?.classList.contains("active");
  const slot = document.querySelector(".product-detail-player-slot");

  if (mobile && detailActive && slot) {
    if (player.parentElement !== slot) slot.appendChild(player);
    return;
  }

  if (player.parentElement !== playerOriginalParent) {
    playerOriginalParent.insertBefore(player, playerOriginalNextSibling);
  }
}
...
if (volumeEl) {
  ["input", "change", "touchend"].forEach(eventName => {
    volumeEl.addEventListener(eventName, e => {
      applyVolume(e.target.value);
    });
  });
  applyVolume(volumeEl.value || volume);
}
